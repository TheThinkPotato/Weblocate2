import { googleResolveType } from "@/Types/Types";
...
export function testValidIp(ip: string): boolean {
  ...
  const classC =
    classCRegEx.test(ip) || classCRegEx2.test(ip) || classCRegEx3.test(ip);
...
export const getIpFromGoogleAnswerArray = (
  response: googleResolveType
): string | undefined => {
...
export const testValidDomain = (domain: string): boolean => {
  const domainRegex = /^(https?:\/\/)?([\w\d-]+\.)*[\w\d-]+\.[\w\d-]+$/; // Regex for domain
  return domainRegex.test(domain); // Return true if valid domain
};

export const removeHttpHttpsFromDomain = (domain: string): string => {
  return domain.replace(/(^\w+:|^)\/\//, ""); // Remove http:// or https:// from domain
};
